...
class PostCell: UITableViewCell {
    ...
    func configure(with post: Post) {
        ...
        if let date = post.createdAt {
            let dateString = DateFormatter.postFormatter.string(from: date)
            let timeString = DateFormatter.timeFormatter.string(from: date)
            dateLabel.text = "Washington, D.C. - \(dateString) at \(timeString)" //DateFormatter.postFormatter.string(from: date)
        }
